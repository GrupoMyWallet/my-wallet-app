    ...
    <div>
        ...
        <div class="flex h-screen bg-gray-100 dark:bg-slate-900 overflow-hidden">
            ...
            <aside :class="[
                ...
                <!-- Top (Logo e título) -->
                <div class="flex items-center gap-4 px-6 py-5 border-b dark:border-slate-700">
                    <MyWalletLogo color="text-black-900 dark:text-white-900"/>
                    <!-- Fechar no mobile -->
                    <button class="ml-auto md:hidden" @click="sidebarOpen = false">
                        <svg class="h-7 w-7 text-gray-400 hover:text-gray-700 dark:hover:text-slate-300" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                ...
                <nav class="flex-1 overflow-y-auto px-4 py-6">
                    <ul class="space-y-2">
                        ...
                        <li v-if="$page.props.jetstream.hasTeamFeatures" class="pt-4 border-t dark:border-slate-700">
                            <div class="text-xs text-gray-400 dark:text-slate-500 px-3 pb-2">Teams</div>
                ...
                <div class="relative px-4 pt-2 pb-5 mt-auto">
                    <button id="profile-menu-btn" @click.stop="showProfileDropdown = !showProfileDropdown"
                        class="w-full flex items-center gap-3 py-2 px-3 rounded-xl text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition font-medium focus:outline-none">
                    ...
                    <transition name="fade-up">
                        <div v-if="showProfileDropdown" id="profile-dropdown"
                            class="absolute left-0 right-0 bottom-full mb-2 z-50 rounded-xl bg-gray-100 dark:bg-slate-800 shadow-lg py-2">
                            <DropdownLink :href="route('profile.show')">Perfil do Usuário</DropdownLink>
                            <DropdownLink v-if="$page.props.jetstream.hasApiFeatures" :href="route('api-tokens.index')">
                                API Tokens</DropdownLink>
                            <div class="border-t mx-2 my-2 dark:border-slate-700"></div>
            ...
            <div class="flex-1 flex flex-col min-w-0 h-screen">
                <!-- Header mobile -->
                <header class="flex items-center h-16 px-4 bg-white dark:bg-slate-800 shadow md:hidden">
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300 focus:outline-none">
                    ...
                    <span class="ml-4 font-semibold text-lg text-gray-700 dark:text-slate-300">{{ title }}</span>
                </header>
